#### Title: BreedingSeason 2022_2023_ResourceUse_Adult_ALLparts
#### Author: Autumn Randall 
#### PI and reference: Dr. DJ McNeil 
#### Date: 9.11.2023
#### Last Editted: 9.11.2023 
##################################################################
...
nobo1 <- read.csv("E:/NOBO Project Data/Analyses/Breeding Season/Summer 2023/Telemetry Data/BreedingSeason2022_2023.csv")
head(nobo1)
nrow(nobo1)

# how many NOBOs in the dataset?
length(unique(nobo1$Bird.ID)) # 961 birds
...
nrow(nobo1) #6095 rows (individual location points)
...
length(unique(nobo1$Bird.ID))# still 624 birds
...
write.csv(dredge1, "E:/NOBO Project Data/Analyses/Breeding Season/Summer 2023/Adult/dredgeAdultBreeding.csv", row.names = FALSE)

top_mod <- glmer(response ~ DTN_road + perc_grassy + ndvi + (1|Bird.ID), family = binomial, data = nobo1)

saveRDS(top_mod, "E:/NOBO Project Data/Analyses/Breeding Season/Summer 2023/Adult/Top_mod_BreedingResourceUse2022_2023.rds")

# ----------------------------------------------------------#

ModelToPredict <- glmer(response ~ DTN_road + perc_grassy + ndvi + (1|Bird.ID), family = binomial, data = nobo1)

newdat <- data.frame(
  ndvi = seq(min(nobo1$ndvi), max(nobo1$ndvi), length.out = 100),
   DTN_road = mean(nobo1$DTN_road), 
   perc_grassy = mean(nobo1$perc_grassy),
  response = 0)
...
# write.csv(newdat, "E:/NOBO Project Data/Analyses/B_NDVI_DataForPlot.csv", row.names = F)
