# Title: Randoms Comparison for camp crane  
# Author: Autumn 
...
cc_shp = subset(OP, course == "campcrane")
plot(cc_shp) # sick it worked 

fall.within.poly <- rands_sp[cc_shp,] # select for just the points that fall in our study area 
...
CC_rands_avg
# write.csv(CC_rands_avg, "./CC_CoverType_AVG")
...
CC = subset(nobo1, CentroidCourses == "campcrane") # birds 
plot(cc_shp) # course <-- as a refresher 

length(unique(CC$Bird.ID)) # 67 birds in FC which seems rather low considering every 
...
CC2 <- CC[0,] # blank data.frame


#### The for() loop:
for(i in 1:length(unique(CC$Bird.ID))){
  
  #i = 19; 57 locations for bird 162.805_231014
  # subset one bird using the subset function
  cc_nobo_i <- subset(CC, Bird.ID == unique(CC$Bird.ID)[i]) # randomly subsetting the data and taking the 100th bird'
  
  # turn nobo_i into spatial. Used for a figure below
  cc_nobo_i_spatial <- SpatialPoints(coords = data.frame("x" = cc_nobo_i$x, "y" = cc_nobo_i$y)) # convert DF to Spatial Points
  crs(cc_nobo_i_spatial) <- CRS("+init=epsg:4326") # define CRS for the spatial points (EPSG 4326 == lat/lon WGS84 etc)
  cc_nobo_i_spatial <- spTransform(cc_nobo_i_spatial, crs(cc_shp)) # make sure it matches OP shapefile: "North American Datum 1983" 
  plot(cc_shp); plot(cc_nobo_i_spatial, add = TRUE, col = "blue") 
  
  # generates random points
  cc_course_i_sh2 <- st_as_sf(cc_shp) # convert to sf
  cc_points1 = st_sample(cc_course_i_sh2, size = nrow(cc_nobo_i) * 2) # generate 2 random points for each "used" point using st_sample()
  cc_points1 <- as_Spatial(cc_points1) # convert back to sp
  plot(cc_shp); plot(cc_points1, add = TRUE, col = "red"); plot(cc_nobo_i_spatial, add = TRUE, col = "blue") # plot to confirm it worked
  cc_points2 <- spTransform(cc_points1, CRS("+init=epsg:4326")) # convert to lat/long because this is what nobo1 uses
  randomcoords <- data.frame(cc_points2@coords) # extract lat/long from the points and convert to data.frame
  # NEW: We need to add random coordinates to a dataframe that will 
  
  # generate pseudo-dates for the randoms
  random_dates <- c(cc_nobo_i$Date, cc_nobo_i$Date)
  
  # Create an empty for randoms suitable for rbind()
  head(CC2) # take a look at NOBO data.frame
  # next, make the new piece of data to be r-binded
  newrandoms <- data.frame("X" = NA, "Bird.ID" = cc_nobo_i$Bird.ID, "ObjectID" = NA, "Date" = random_dates, 
                           ...
                           "breedingseasonCov" = NA, "yearBinary" = NA, "n" = cc_nobo_i$n,
                           "CentroidCourses" = cc_nobo_i$CentroidCourses)
  
  CC2 <- rbind(CC2, newrandoms)
  
  # progress bar aka DJs child 
  compl <- round(i/length(unique(CC$Bird.ID))*15,0)
  cat(paste0("\r [",strrep("🐣", compl), strrep("⚪️", 15-compl),"] ", round(100*i/length(unique(CC$Bird.ID)),0), "% complete, bird #", i, " done"))
}

CC2 # our dataframe for the randoms for fence cove 
unique(CC2$Location.Type) # random
nrow(CC2) #4922 --> this should be good. 2 * the number of originals. 2461 * 2 = 6148
