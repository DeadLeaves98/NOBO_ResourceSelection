# Title: Randoms Comparison for allens creek 
# Author: Autumn 
...
ac_shp = subset(OP, course == "allenscreek")
plot(ac_shp) # sick it worked 

fall.within.poly <- rands_sp[ac_shp,] # select for just the points that fall in our study area 
...
# write.csv(AC_rands_avg, "./AC_CoverType_AVG")
...
AC = subset(nobo1, CentroidCourses == "allenscreek") # birds 
plot(ac_shp) # course <-- as a refresher 

length(unique(AC$Bird.ID)) # 67 birds in FC which seems rather low considering every 
...
AC2 <- AC[0,] # blank data.frame


#### The for() loop:
for(i in 1:length(unique(AC$Bird.ID))){
  
  #i = 70; 60 locations for bird 162.407_220625     
  # subset one bird using the subset function
  ac_nobo_i <- subset(AC, Bird.ID == unique(AC$Bird.ID)[i]) # randomly subsetting the data and taking the 100th bird'
  
  # turn nobo_i into spatial. Used for a figure below
  ac_nobo_i_spatial <- SpatialPoints(coords = data.frame("x" = ac_nobo_i$x, "y" = ac_nobo_i$y)) # convert DF to Spatial Points
  crs(ac_nobo_i_spatial) <- CRS("+init=epsg:4326") # define CRS for the spatial points (EPSG 4326 == lat/lon WGS84 etc)
  ac_nobo_i_spatial <- spTransform(ac_nobo_i_spatial, crs(ac_shp)) # make sure it matches OP shapefile: "North American Datum 1983" 
  plot(ac_shp); plot(ac_nobo_i_spatial, add = TRUE, col = "blue") 
  
  # generates random points
  ac_course_i_sh2 <- st_as_sf(ac_shp) # convert to sf
  ac_points1 = st_sample(ac_course_i_sh2, size = nrow(ac_nobo_i) * 2) # generate 2 random points for each "used" point using st_sample()
  ac_points1 <- as_Spatial(ac_points1) # convert back to sp
  plot(ac_shp); plot(ac_points1, add = TRUE, col = "red"); plot(ac_nobo_i_spatial, add = TRUE, col = "blue") # plot to confirm it worked
  ac_points2 <- spTransform(ac_points1, CRS("+init=epsg:4326")) # convert to lat/long because this is what nobo1 uses
  randomcoords <- data.frame(ac_points2@coords) # extract lat/long from the points and convert to data.frame
  # NEW: We need to add random coordinates to a dataframe that will 
  
  # generate pseudo-dates for the randoms
  random_dates <- c(ac_nobo_i$Date, ac_nobo_i$Date)
  
  # Create an empty for randoms suitable for rbind()
  head(AC2) # take a look at NOBO data.frame
  # next, make the new piece of data to be r-binded
  newrandoms <- data.frame("X" = NA, "Bird.ID" = ac_nobo_i$Bird.ID, "ObjectID" = NA, "Date" = random_dates, 
                           ...
                           "breedingseasonCov" = NA, "yearBinary" = NA, "n" = ac_nobo_i$n,
                           "CentroidCourses" = ac_nobo_i$CentroidCourses)
  
  AC2 <- rbind(AC2, newrandoms)

  # progress bar aka DJs child 
  compl <- round(i/length(unique(AC$Bird.ID))*15,0)
  cat(paste0("\r [",strrep("🐣", compl), strrep("⚪️", 15-compl),"] ", round(100*i/length(unique(AC$Bird.ID)),0), "% complete, bird #", i, " done"))
}

AC2 # our dataframe for the randoms for fence cove 
unique(AC2$Location.Type) # random
nrow(AC2) # 9210 --> this should be good. 2 * the number of originals. 4605 * 2 = 9110
...
# EXTRACTING RASTER VALUES
ac2_sp <- SpatialPoints(coords = data.frame("x" = AC2$x, "y" = AC2$y)) # convert DF to Spatial Points
crs(ac2_sp) <- CRS("+init=epsg:4326") # define CRS for the spatial points (EPSG 4326 == lat/lon WGS84 etc)
plot(ac2_sp)
# read in rasters
# hard copied into it because unsure if git can handle
list.files("E:/NOBO Project Data/Analyses/Breeding Season/Summer 2023/Adult/rasters")
...
# reproject nobo and extract
ac2_sp1 <- spTransform(ac2_sp, crs(NDVI)) # transform to NAD83
plot(NDVI); plot(ac2_sp1, add = TRUE)
ndv1_ex <- raster::extract(x = NDVI, y = ac2_sp1)
perc_mpine_ex <- raster::extract(x = perc_mpine, y = ac2_sp1)
perc_grassy_ex <- raster::extract(x = perc_grassy, y = ac2_sp1)
perc_decid_ex <- raster::extract(x = perc_decid, y = ac2_sp1)
perc_bf_ex <- raster::extract(x = perc_bf, y = ac2_sp1)
perc_water_ex <- raster::extract(x = perc_water, y = ac2_sp1)

ac2_sp2 <- spTransform(ac2_sp, crs(DTN_road)) # transform to pseudo-mercator
plot(DTN_road); plot(ac2_sp2, add = TRUE)
DTN_road_ex <- raster::extract(x = DTN_road, y = ac2_sp2)
DTN_mpine_ex <- raster::extract(x = DTN_mpine, y = ac2_sp2)
DTN_grassy_ex <- raster::extract(x = DTN_grassy, y = ac2_sp2)
DTN_decid_ex <- raster::extract(x = DTN_decid, y = ac2_sp2)
DTN_bf_ex <- raster::extract(x = DTN_bf, y = ac2_sp2)
DTN_water_ex <- raster::extract(x = DTN_water, y = ac2_sp2)

# add columns to NOBO and export
AC2$ndvi <- ndv1_ex
AC2$perc_mpine <- perc_mpine_ex
AC2$perc_grassy <- perc_grassy_ex
AC2$perc_decid <- perc_decid_ex
AC2$perc_bf <- perc_bf_ex
AC2$perc_water <- perc_water_ex
AC2$DTN_road <- DTN_road_ex
AC2$DTN_mpine <- DTN_mpine_ex
AC2$DTN_grassy <- DTN_grassy_ex
AC2$DTN_decid <- DTN_decid_ex
AC2$DTN_bf <- DTN_bf_ex
AC2$DTN_water <- DTN_water_ex 
...
# boxwhisker plots 
head(AC_rands_avg) # the mean cover type for fence cove AKA what we will be comparing 
# our randoms generated per bird to to make sure the randoms are accurately representing 
# the course 

head(AC2)
#### NDVI ----
ndvi_bw_allenscreek = boxplot(AC2$ndvi, 
                            main = "Random point distribution: NDVI in AC", 
                            ylab = "Average", 
                            xlab = "NDVI") 
abline(h = 0.437, col = "purple")

#### % mpine ----
perc_mpine_bw_allenscreek = boxplot(AC2$perc_mpine, 
                                  main = "Randoms for % mpine in AC", 
                                  ylab = "Average", 
                                  xlab = "% mpine") 
abline(h = 0.430, col = "purple")

#### % grassy ----
perc_grassy_bw_allenscreek = boxplot(AC2$perc_grassy, 
                                   main = "Randoms for % grassy in AC", 
                                   ylab = "Average", 
                                   xlab = "% grassy") 
abline(h = 0.250, col = "purple")

#### % decid ----
perc_decid_bw_allenscreek = boxplot(AC2$perc_decid, 
                                  main = "Randoms for % decid in AC", 
                                  ylab = "Average", 
                                  xlab = "% decid") 
abline(h = 0.177, col = "purple")

#### % BF ----
perc_bf_bw_allenscreek = boxplot(AC2$perc_bf, 
                               main = "Randoms for % bf in AC", 
                               ylab = "Average", 
                               xlab = "% bf") 
abline(h = 0.0910, col = "purple")

#### % water ----
perc_water_bw_allenscreek = boxplot(AC2$perc_water, 
                                  main = "Randoms for % water in AC", 
                                  ylab = "Average", 
                                  xlab = "% water") 
abline(h = 0.0470, col = "purple") #TO NOTE: WILL BE REMOVING THIS FROM THE DATA ANYWAY 


#### dtn road  ----
dtn_road_bw_allenscreek = boxplot(AC2$DTN_road, 
                                main = "Randoms for DTN road in AC", 
                                ylab = "Average", 
                                xlab = "DTN road") 
abline(h = 27.3, col = "purple")

#### dtn mpine  ----
dtn_mpine_bw_allenscreek = boxplot(AC2$DTN_mpine, 
                                 main = "Randoms for DTN mpine in AC", 
                                 ylab = "Average", 
                                 xlab = "DTN mpine") 
abline(h = 11.2, col = "purple")

#### dtn grassy   ----
dtn_grassy_bw_allenscreek = boxplot(AC2$DTN_grassy, 
                                  main = "Randoms for DTN grassy in AC", 
                                  ylab = "Average", 
                                  xlab = "DTN grassy") 
abline(h = 27.2, col = "purple")

#### dtn decid  ----
dtn_mpine_bw_allenscreek = boxplot(AC2$DTN_decid, 
                                 main = "Randoms for DTN decid in AC", 
                                 ylab = "Average", 
                                 xlab = "DTN decid") 
abline(h = 37.9, col = "purple")

#### dtn bf  ----
dtn_bf_bw_allenscreek = boxplot(AC2$DTN_bf, 
                              main = "Randoms for DTN bf in AC", 
                              ylab = "Average", 
                              xlab = "DTN bf") 
abline(h = 37.9, col = "purple")

#### dtn water  ----
dtn_water_bw_allenscreek = boxplot(AC2$DTN_water, 
                                 main = "Randoms for DTN water in AC", 
                                 ylab = "Average", 
                                 xlab = "DTN water") 
abline(h = 148, col = "purple")
